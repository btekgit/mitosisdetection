...
a = cascade.CascadedBooster(base_clf=clf, max_layers=20, priors=[0.90, 0.01])
a.fit(X1, Y1)

t1 = time.clock()
pred, prob = a.predict_proba(X)
tp = ((Y == 1) & (pred>=0)).sum() / float((Y==1).sum())
fp = 1 - ((Y == 0) & (pred<0)).sum() / float((Y==0).sum())
pr = ((Y == 1) & (pred>=0)).sum() / float((pred>=0).sum())
print "Tp: ", tp, "Fp: ", fp, "Pr: ", pr
t2 = time.clock()
print "Elapsed Time: ", t2-t1

pred, prob = a.predict_proba(X2)
tp = ((Y2 == 1) & (prob[:,1]>=0.5)).sum() / float((Y2==1).sum())
fp = 1 - ((Y2 == 0) & (prob[:,1]<0.5)).sum() / float((Y2==0).sum())
pr = ((Y2 == 1) & (prob[:,1]>=0.5)).sum() / float((pred>=0).sum())
print "Tp: ", tp, "Fp: ", fp, "Pr: ", pr
# test on whole
...
t1 = time.clock()
pred, prob = a.predict_proba(X2)
tp = ((Y2 == 1) & (pred>=0)).sum() / float((Y2==1).sum())
fp = 1 - ((Y2 == 0) & (pred<0)).sum() / float((Y2==0).sum())
pr = ((Y2 == 1) & (prob[:,1]>=0.5)).sum() / float((pred>=0).sum())
print "Tp: ", tp, "Fp: ", fp, "Pr: ", pr
t2 = time.clock()
print "Elapsed Time: ", t2-t1
