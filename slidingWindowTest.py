...
CLASSIFIERFILE_CONT = 'random_forest_trained_cum_samples.pkl'
RESULTS_FILE = './mitosisData/bootstrapCollectedSamples/training_track.pkl'
...
sampleCollectStepSizeList = [21,11,10,9,8,7,6,5,4,6,5,4,3,2,3,2,1,1,1,1]
...
for stpSize in sampleCollectStepSizeList:
    ...
    #       reset sample counter
        ...
        if k == NUMBEROFTRAININGIMAGES:
            ...
            _ = joblib.dump(training_results, RESULTS_FILE, compress=9)
    ...
    if TRAININGMODE == 0:
		...
		#posIndex = allCollectedSampleLabels==1
		#wbalance = dict({0: nposAll/nAll, 1: nnegAll/nAll})
		allCollectedSampleX, allCollectedSampleLabels = balanceLabels(allCollectedSampleSetX, allCollectedSampleLabels)
            clf, oob_error = trainIncrementally(clf,
                                                allCollectedSampleSetX,
                                                allCollectedSampleLabels, class_weights=wbalance)
            allCollectedSampleSetX = []
            allCollectedSampleLabels = []
            nNegativeSamples = 0
            nPositiveSamples = 0
