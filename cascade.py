...
class CascadedBooster(BaseEstimator, ClassifierMixin):
    def __init__(self, targetTP=0.99, targetFP=1e-3,
        ...
        # layer targets are set based on the final layer target 
        self.layerTargetTP = (np.ones(self.max_layers)
                            * pow(self.targetTP, 1./self.max_layers))
        self.layetTargetFP = (np.ones(self.max_layers) 
                            * pow(self.targetFP, 1./self.max_layers))
    ...
    def fit(self, trnX, trnY, valX=None, valY=None, trnValRatio):
        """ Fit the cascaded boosting model """

        trnX, trnY = trnX.copy(), trnY.copy()
        if valX:
            valX = valX or trnX.copy()
            valY = valY or trnY.copy()
        else:
            trnX, trnY, valX, valY = 
        ...
        for i in range(self.max_layers):
            ...
            bias = _set_bias(clf, trnX, trnY, self.targetTP,
                             self.targetFP, nNeg)

            self.estimators_.append(clf)
            self.bias_.append(bias)

            Yp = clf.decision_function(valX).ravel() >= bias
            F[-1] = 1.0 * ((trnY == 0) & Yp).sum() / nNeg
            rc = 1.0 * ((trnY == 1) & Yp).sum() / nPos
            if self.verbose > 0:
    ...
    def __str__ (self):
        return ("Cascade of "+ str(len(self.estimators_))+ 
               "TP: "+str(self.targetTP) + "FP: "+ str(self.targetFP))
    
    def add_cascade_layer(self, X, Y, fitParam=None):
